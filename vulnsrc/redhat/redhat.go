...
func Update(dir string, updatedFiles map[string]struct{}) error {
	...
	if err != nil {
		...
		log.Println("Red Hat: no updated file")
		return nil
	}
	log.Printf("Red Hat updated files: %d", len(targets))
	...
	err := db.BatchUpdate(func(tx *bolt.Tx) error {
		for _, cve := range cves {
			for _, affected := range cve.AffectedRelease {
				...
				if !util.StringInSlice(affected.ProductName, targetPlatforms) {
					continue
				}
			...
			for _, pkgState := range cve.PackageState {
				...
				if !util.StringInSlice(platformName, targetPlatforms) {
					continue
				}
				if !util.StringInSlice(pkgState.FixState, targetStatus) {
					continue
				}
