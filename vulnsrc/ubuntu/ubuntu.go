...
func Update(dir string, updatedFiles map[string]struct{}) error {
	...
	if err != nil {
		...
		log.Println("Ubuntu: no updated file")
		return nil
	}
	log.Printf("Ubuntu OVAL updated files: %d", len(targets))
		...
		for _, cve := range cves {
			for packageName, patch := range cve.Patches {
				...
				for release, status := range patch {
					if !util.StringInSlice(status.Status, targetStatus) {
						continue
					}
