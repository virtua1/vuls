...
func CloneOrPull(url, repoPath string) (map[string]struct{}, error) {
	...
	if exists {
		...
		if !util.IsCommandAvailable("git") {
			log.Println("Recommend installing git (if not, DB update is very slow)")
		}
		log.Println("remove an existed directory")
...
func pull(repoPath string) ([]string, error) {
	...
	log.Println("Retrieve the branch being pointed by HEAD")
	ref, err := r.Head()
	if err != nil {
		return nil, xerrors.Errorf("failed to get HEAD: %w", err)
	}

	log.Println("Get the working directory for the repository")
	w, err := r.Worktree()
	if err != nil {
		return nil, xerrors.Errorf("failed to get the working directory: %w", err)
	}

	log.Println("Pull the latest changes from the origin remote and merge into the current branch")
	...
	log.Println("Retrieve the commit history")
	commits, err := r.Log(&git.LogOptions{})
	if err != nil {
		return nil, xerrors.Errorf("error in git log: %w", err)
	}

	log.Println("Detect the updated files")
